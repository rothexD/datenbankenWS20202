...
CREATE OR REPLACE PROCEDURE sp_create_mitarbeiter(v_Name VARCHAR2,Geburtsdatum TIMESTAMP,v_Adresse VARCHAR2,n_Sozialversicherungsnummer NUMBER,n_PLZ NUMBER,n_Gehaltsstufe NUMBER,n_MitarbeiterRolle NUMBER)
AS
	n_newPersonID NUMBER;
BEGIN
	SAVEPOINT CreateMitarbeiter;
	select person_id_seq.NEXTVAL INTO n_newPersonID FROM dual;
	INSERT INTO Person VALUES(n_newPersonID,v_Name,Geburtsdatum,v_Adresse,n_PLZ);
...
CREATE OR REPLACE PROCEDURE sp_create_kunde(v_NameOf VARCHAR2,Geburtsdatum TIMESTAMP,v_Adresse VARCHAR2,n_PLZ NUMBER,v_Email VARCHAR2,n_Kreditkartenummer NUMBER)
AS
n_newPersonID NUMBER;
BEGIN
	SAVEPOINT CreateKunde;
	select person_id_seq.NEXTVAL INTO n_newPersonID FROM dual;
	INSERT INTO Person VALUES(n_newPersonID,v_NameOf,Geburtsdatum,v_Adresse,n_PLZ);
	INSERT INTO Kunde VALUES(n_newPersonID,v_Email,n_Kreditkartenummer,n_newPersonID + 10000,0);
EXCEPTION
	WHEN DUP_VAL_ON_INDEX THEN
